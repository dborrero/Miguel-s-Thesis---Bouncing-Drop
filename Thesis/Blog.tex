\chapter*{Blog}
	...
	\item{Order Accelerometer.    Miguel 11/4/14}
...
We can rewrite $R$ in a way that is helpful if we say that $P(\mathbf{x},t)=R(\mathbf{x},t)^2=|\psi(\mathbf{x},t)|^2$ where $P(\mathbf{x},t)$ is the probability density. Plugging this in to \refeq{dS/dt} and rewriting in into a provocative form makes

MATH?

\begin{equation}
\label{BohmHamiltonian}
- \frac{\partial S}{\partial t} = \frac{(\nabla S)^2}{2 m} + V(\mathbf{x},t) + \frac{\hbar^2}{4m}\left[\frac{1}{2} \frac{\nabla^2 P}{P} - \frac{(\nabla P)^2}{P^2}\right]
\end{equation}
...
U(\mathbf{x}) =  \frac{\hbar^2}{4m}\left[\frac{1}{2} \frac{\nabla^2 P}{P} - \frac{(\nabla P)^2}{P^2}\right] = \frac{\hbar^2}{2m}\frac{\nabla^2 R}{R}
\end{equation}
...
Plugging in the probability density into \refeq{dR/dt} also gives us something quite interesting.
