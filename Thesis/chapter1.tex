...
Constraining a walker to a circular region, Harris et al. tracked the motion of the walker over a period of time \cite{Harris2013}. At the high-memory, chaotic motion regime, the droplet was allowed to walk freely while its position was tracked (\refFig{stat}(a)) and translated into the histogram (\refFig{stat}(b)). The histogram provides the probability of finding the walker at a specific location within the corral, serving the same purpose as de Broglie's statistical wave described by the Schr\"{o}dinger equation. In the Copenhagen interpretation of quantum mechanics however, this statistical wave (called the wavefunction) \textit{is} the location particle. 
